 ...
 * @author kom1
 */
public class AvailableTableController implements Initializable {
    ...
    private void makeItAvailable(MouseEvent event) {
        ...
        if(res>0){
  		  ...
  		  alert.setTitle("Dane zaktualizowane");
  		  alert.setHeaderText("Okno dialogowe");
  		  alert.setContentText("Stan stolika uaktualniony");
  		  alert.showAndWait();
                  loadAllTables("SELECT * FROM tables");
        }else{
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
